...
class TestCommentModel:
    ...
    def test_create_comment(self, app, client, auth):
        ...
        with app.app_context():
            ...
            post = PostService.create(
                author_id=1, title="Тестовый пост", content="Содержание поста"
            )

            # Создаём комментарий
            comment = CommentService.create(
                author_id=1, post_id=post.id, content="Это тестовый комментарий"
            )
    ...
    def test_find_comment_by_id(self, app, auth):
        ...
        with app.app_context():
            ...
            post = PostService.create(
                author_id=1, title="Тестовый пост", content="Содержание"
            )

            # Создаём комментарий
            created_comment = CommentService.create(
                author_id=1, post_id=post.id, content="Тестовый комментарий"
            )
    ...
    def test_delete_foreign_comment(self, app, auth):
        ...
        with app.app_context():
            ...
            comment = CommentService.create(
                author_id=1, post_id=post.id, content="Комментарий пользователя 1"
            )
    ...
    def test_get_count_by_post_id(self, app, auth):
        ...
        with app.app_context():
            ...
            # Создаём несколько комментариев
            CommentService.create(author_id=1, post_id=post.id, content="Комментарий 1")
            CommentService.create(author_id=1, post_id=post.id, content="Комментарий 2")
...
class TestCommentRoutes:
    ...
    def test_add_comment_as_logged_in_user(self, client, auth):
        ...
        response = client.post(
            ...
            data={"content": "Новый комментарий", "csrf_token": "test-csrf-token"},
...
class TestCommentModelMethods:
    ...
    def test_author_display_name(self):
        ...
        comment = Comment(author_username="testuser", author_discriminator=42)
