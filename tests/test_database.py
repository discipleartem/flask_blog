        ...
        with app.app_context():
            ...
            if user:
                ...
                if post:
                    ...
                    # READ
                    comment = db.execute(
                        'SELECT * FROM comment WHERE content = ?',
                        ('Test comment',)
                    ).fetchone()
...
class TestDatabaseConstraints:
    ...
    def test_unique_username_discriminator(self, app):
        ...
        with app.app_context():
            ...
            # Попытка создать второго пользователя с тем же username и discriminator
            try:
...
class TestDatabasePerformance:
    ...
    def test_batch_insert_performance(self, app):
        ...
        with app.app_context():
            ...
            # Массовая вставка пользователей (меньше пользователей для скорости)
            start_time = time.time()
            ...
            db.execute(
                'DELETE FROM user WHERE username LIKE "perf_test_%"'
            )
            db.commit()
