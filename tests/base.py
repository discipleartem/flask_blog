...
class BaseTestCase:
    ...
    def assert_csrf_validation(self, token, expected=True):
        ...
        assert result is expected, f"CSRF validation failed: expected {expected}, got {result}"
    ...
    def create_test_user_data(self, username="test_user", discriminator=1234, password="test_pass"):
    ...
    def assert_form_validation_error(self, response, error_keywords=None):
        ...
        error_indicators = ['alert-danger', 'ошибка', 'обязательно', 'должно быть', 'формат', 'неверный']
...
class DatabaseTestHelper:
    ...
    @staticmethod
    def create_user_with_post(db, username, discriminator, password, post_title, post_content):
    ...
    def cleanup_test_data(db, usernames=None, user_ids=None):
        ...
        if usernames:
            ...
            db.execute(f'DELETE FROM user WHERE username IN ({placeholders})', usernames)
