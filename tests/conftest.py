...
if not sys.warnoptions:
    ...
    warnings.filterwarnings(
        "ignore", category=DeprecationWarning, message=".*default timestamp converter.*"
    )
...
def _create_test_data():
    ...
    test_posts = [
        ...
        (
            "Пост для просмотра",
            "Этот пост используется для тестирования просмотра",
            user_id,
        ),
    ...
    test_comments = [
        ...
        (
            1,
            user_id,
            "Второй комментарий к первому посту",
        ),  # Еще один комментарий к посту 1
...
def app():
    ...
    app, db_fd, db_path = _create_test_app({"WTF_CSRF_ENABLED": False})
...
def app_with_csrf():
    ...
    app, db_fd, db_path = _create_test_app({"WTF_CSRF_ENABLED": True})
...
def security_app():
    ...
    app, db_fd, db_path = _create_test_app(
        {"WTF_CSRF_ENABLED": True, "SECRET_KEY": "security-test-key"}
    )
    ...
    def login(
        ...
        if app.config.get("WTF_CSRF_ENABLED", False):
            ...
            csrf_match = re.search(
                r'name="csrf_token"[^>]*value="([^"]+)"', page_content, re.DOTALL
            )
            if csrf_match:
                login_data["csrf_token"] = csrf_match.group(1).strip()

        return self._client.post(
            "/auth/login", data=login_data, follow_redirects=follow_redirects
        )
    ...
    def register(
        ...
        if app.config.get("WTF_CSRF_ENABLED", False):
            ...
            csrf_match = re.search(
                ...
                register_page.get_data(as_text=True),
                re.DOTALL,
            )
            if csrf_match:
        ...
        response = self._client.post(
            "/auth/register", data=register_data, follow_redirects=follow_redirects
        )
        return response
