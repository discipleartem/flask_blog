...
**Обязательно** заполните `.env` на сервере **до** первого старта WSGI. В non-debug режиме (`FLASK_DEBUG` не `1`) factory отказывается стартовать, если `SECRET_KEY` или `ADMIN_PASSWORD` остались известными небезопасными дефолтами (`dev-change-me` / `admin`) — hard fail с `RuntimeError` и записью в лог (см. `create_app` / `_assert_secure_production_secrets`).
...
| Переменной нет ни в окружении, ни в `.env` | Дефолт из `Config` (`SECRET_KEY=dev-change-me`, `ADMIN_PASSWORD=admin`) — **на проде старт упадёт** |
